...
#define spec_elem_alltoallv_proclists_ip(_b_, _xb_, _sc_, _sd_, _nsp_, _sp_, _rc_, _rd_, _nrp_, _rp_, _s_, _r_, _c_) do { \
    ZMPI_Alltoallv_inplace_aux = (_xb_)->buf; \
    ZMPI_Alltoallv_inplace_aux_size = zmpil_nextent((_xb_)->count, &(_xb_)->zmpil_type); \
    ZMPI_Alltoallv_inplace_proclists(MPI_IN_PLACE, (_sc_), (_sd_), (_b_)->mpi_type, (_nsp_), (_sp_), (_b_)->buf, (_rc_), (_rd_), (_b_)->mpi_type, (_nrp_), (_rp_), (_c_)); \
  } while (0)
#endif

#define DECLARE_SPEC_ELEM_ALLTOALLW_INDEXED_DB  struct { spint_t i, j; MPI_Aint rtype_lb, rtype_extent; } specwd;

#define spec_elem_alltoallw_indexed_db(_sb_, _sc_, _sd_, _st_, _rb_, _rc_, _rd_, _rt_, _ix_, _ic_, _id_, _s_, _r_, _c_) Z_MOP( \
  MPI_Type_get_extent((_rb_)->mpi_type, &specwd.rtype_lb, &specwd.rtype_extent); \
  for (specwd.i = 0; specwd.i < (_s_); ++specwd.i) { \
    MPI_Type_create_indexed_block((_ic_)[specwd.i], 1, &(_ix_)[(_id_)[specwd.i]], (_sb_)->mpi_type, &(_st_)[specwd.i]); \
    MPI_Type_commit(&(_st_)[specwd.i]); \
    (_rd_)[specwd.i] *= specwd.rtype_extent; \
    (_rt_)[specwd.i] = (_rb_)->mpi_type; \
  } \
  MPI_Alltoallw((_sb_)->buf, (_sc_), (_sd_), (_st_), (_rb_)->buf, (_rc_), (_rd_), (_rt_), (_c_)); \
  for (specwd.i = 0; specwd.i < (_s_); ++specwd.i) { \
    MPI_Type_free(&(_st_)[specwd.i]); \
  } )

#ifdef HAVE_ZMPI_ALLTOALLX_PROCLISTS
#define spec_elem_alltoallw_indexed_proclists_db(_sb_, _sc_, _sd_, _st_, _nsp_, _sp_, _rb_, _rc_, _rd_, _rt_, _nrp_, _rp_, _ix_, _ic_, _id_, _s_, _r_, _c_) Z_MOP( \
  for (specwd.i = 0; specwd.i < (_nsp_); ++specwd.i) { \
    specwd.j = (_sp_)[specwd.i]; \
    MPI_Type_create_indexed_block((_ic_)[specwd.j], 1, &(_ix_)[(_id_)[specwd.j]], (_sb_)->mpi_type, &(_st_)[specwd.j]); \
    MPI_Type_commit(&(_st_)[specwd.j]); \
  } \
  MPI_Type_get_extent((_rb_)->mpi_type, &specwd.rtype_lb, &specwd.rtype_extent); \
  for (specwd.i = 0; specwd.i < (_nrp_); ++specwd.i) { \
    specwd.j = (_rp_)[specwd.i]; \
    (_rd_)[specwd.j] *= specwd.rtype_extent; \
    (_rt_)[specwd.j] = (_rb_)->mpi_type; \
  } \
  ZMPI_Alltoallw_proclists((_sb_)->buf, (_sc_), (_sd_), (_st_), (_nsp_), (_sp_), (_rb_)->buf, (_rc_), (_rd_), (_rt_), (_nrp_), (_rp_), (_c_)); \
  for (specwd.i = 0; specwd.i < (_nsp_); ++specwd.i) { \
    specwd.j = (_sp_)[specwd.i]; \
    MPI_Type_free(&(_st_)[specwd.j]); \
  } )
#endif
